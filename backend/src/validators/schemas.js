...
const habitFields = z.object({ name: z.string().trim().min(1).max(100), description: z.string().trim().max(1000).nullable().optional(), scheduleType: z.enum(['DAILY', 'CUSTOM']), weekdays: z.array(z.number().int().min(0).max(6)).max(7).optional().default([]) });
export const habitSchema = habitFields.superRefine((data, ctx) => { if (data.scheduleType === 'CUSTOM' && !data.weekdays.length) ctx.addIssue({ code: 'custom', message: 'Choose at least one day for a custom schedule', path: ['weekdays'] }); if (new Set(data.weekdays).size !== data.weekdays.length) ctx.addIssue({ code: 'custom', message: 'Days must be unique', path: ['weekdays'] }); });
export const habitPatchSchema = habitFields.partial().superRefine((data, ctx) => { if (data.scheduleType === 'CUSTOM' && data.weekdays && !data.weekdays.length) ctx.addIssue({ code: 'custom', message: 'Choose at least one day', path: ['weekdays'] }); });
